package org.csi.yucca.speedalignment.single
...
object HiveToSolrSingleDataset {
  ...
  try {
		...
		LOG.info("[[HiveToSolrSingleDataset::main]] BEGIN")
			...
			val startFrom:Integer =Integer.parseInt(args(4))
			
		  var prjName:String = "" //il parametro prjName non e' obbligatorio
			if (args.length == 6)
			     prjName = args(5)
		  ...
		  if (! (WRITEMODE_NOWRITE.equalsIgnoreCase(toWrite)  ||  WRITEMODE_APPEND.equalsIgnoreCase(toWrite) || WRITEMODE_OVERWRITE.equalsIgnoreCase(toWrite)  ) ) {
			...
			LOG.info("[[HiveToSolrSingleDataset::main]] recupero dss info")
			val ds=adminDel.getDatasets4PromotionByIdDataset( sparkContext,idDataset)
			LOG.info("[[HiveToSolrSingleDataset::main]] fatto")
			
			retCode=writeDataset(solrDelegate,sqlContextHive,sparkContext,ds,stepCnt,toWrite,forceCollectionTO,startFrom,prjName)
  ...
  def writeDataset(solrDelegate:SolrDelegate,sqlContextHive:HiveContext,sparkContext:SparkContext,
      ...
      startFrom:Integer,
      prjName:  String) : Integer = {
    LOG.info("[[HiveToSolrSingleDataset::main::writeDataset]]BEGIN")
      ...
      var countAfterDele = countInitSolr
			if ( WRITEMODE_OVERWRITE.equalsIgnoreCase(toWrite)) {
			  LOG.info("[[HiveToSolrSingleDataset::main]]   deleting from hdfs --> tenant:" +ds.organizationCode + "   domain: "+ds.dataDomain  + "   subdomain: "+ ds.codSubDomain + " dssubtype:" +ds.subType + " dscode:" +ds.datasetCode+ " streamcode:" + ds.streamCode + " slug:" + ds.vESlug)
	...
	/*		
      ...
      var logRow = hiveContext.createDataFrame(Seq((DateTime.now().toString(), 
	        ...
	        recWritten,
	        elapsedTime,
	        ds.datasetCode))).toDF("timeoperation","id_application_yarn","spark_app_name","prjname","publish_type","hivetable","solr_destination","record_read","record_write","elapsed_type","dataset_code")
