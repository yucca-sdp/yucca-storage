...
case class CliOptions (
  ...
  try {
		...
		LOG.info("[[HiveToCsvFullTenant::main]] BEGIN")
    ...
    parser.parse( args, CliOptions()) match {
      ...
      case Some(opts) => {
        ...
        coordinatorName=opts.coordinatorName
		  ...
		  LOG.info("[[HiveToCSvFullTenant::main]] tenantCode --> "+tenantCode)
		  LOG.info("[[HiveToCSvFullTenant::main]] toWrite --> "+toWrite)
		  LOG.info("[[HiveToCSvFullTenant::main]] logStatisticsToHive --> "+logStatisticsToHive)
		  LOG.info("[[HiveToCSvFullTenant::main]] prjName --> "+prjName)
		  LOG.info("[[HiveToCSvFullTenant::main]] coordinatorName --> "+coordinatorName)
		  ...
		  if (! (WRITEMODE_NOWRITE.equalsIgnoreCase(toWrite)  ||  WRITEMODE_APPEND.equalsIgnoreCase(toWrite) || WRITEMODE_OVERWRITE.equalsIgnoreCase(toWrite)  ) ) {
			...
			LOG.info("[[HiveToCsvFullTenant::main]] datasets --> "+datasets.size)
     ...
     var dfStats:DataFrame = null;
      ...
      if (logStatisticsToHive==true && dfStats!=null && dfStats.count()>0) {
	    ...
	    case e: Exception => LOG.error("[[HiveToCSvFullTenant::main]]"+e, e)
	    throw e
	  } finally {
			LOG.info("[[HiveToCSvFullTenant::main]] END")
    ...
    System.exit(retCode)
	 ...
	 def writeCsv(sqlContextHive:HiveContext,sparkContext:SparkContext,
      ...
      val dsType = ds.subType match { 
			...
			LOG.info("[[HiveToCsvFullTenant::main]]strCols = "+strCols)
			LOG.info("[[HiveToCsvFullTenant::main]]hive = "+ds.dbHiveSchema+"."+ds.dbHiveTable)
      ...
      _hiveTable.withColumnRenamed(_column.toString, _column.toString.toLowerCase.replaceFirst("(_[^_]+$)", "")))
			...
			LOG.info("[[HiveToCSvFullTenant::main]]   Numero Record hive da scrivere  --> "+numrecordTowrite)
      
			var hdfsHelper= new HDFSHelper(null)
			//CANCELLAZIONE IN CASO DI REPLACE
		
			if ( WRITEMODE_OVERWRITE.equalsIgnoreCase(toWrite)) {
			  LOG.info("[[HiveToCsvFullTenant::main]]   deleting from hdfs --> tenant:" +ds.organizationCode + "   domain: "+ds.dataDomain  + "   subdomain: "+ ds.codSubDomain + " dssubtype:" +ds.subType + " dscode:" +ds.datasetCode+ " streamcode:" + ds.streamCode + " slug:" + ds.vESlug)
  		if (!(WRITEMODE_NOWRITE.equalsIgnoreCase(toWrite))) hdfsHelper.deleteDataSetForCsv(sparkContext.hadoopConfiguration, ds.organizationCode, ds.dataDomain, ds.codSubDomain, ds.subType, ds.datasetCode, ds.streamCode, ds.vESlug)
				LOG.info("[[HiveToCsvFullTenant::main]] deleting")
				Thread.sleep(10000)
